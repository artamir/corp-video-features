...
Если АргументыКоманднойСтроки.Количество() < 2 Тогда
	...
	Лог.Ошибка("    Параметр 1: Файл источник");
	Лог.Ошибка("    Парамерт 2: Выходящий файл");
	//Возврат;
...
СоответствиеВходящийФайл = ПрочитатьФайлJSON(ПутьВходящийФайл);
ПреобразоватьПараметрыКоторыеНачинаютсяСТочкиКНормальнымПутям(СоответствиеВходящийФайл);

ТекстФайла = РаботаСФайлами.ОБъектВJson(СоответствиеВходящийФайл);
РаботаСФайлами.ЗаписатьФайл(ПутьИсходящийФайл, ТекстФайла);