...
Функция ВыполнитьСкрипт(СтрокаКоманды)
	...
	Лог.Отладка("------------------------------------------------------------------");
	Лог.Отладка(ИмяВременногоФайлаКоманда);
	Лог.Отладка(СтрокаКоманды);
	Лог.Отладка("retCode = " + retCode);
...
Функция ПреобразоватьПутьСТочкамиКНормальномуПути(ОригСтр)
	...
	retCode = ВыполнитьСкрипт(Команда);
...
Процедура ВыполнитьЗапускОднойСборки(ПараметрыСборки)
	...
	//@# Начало МА
	Лог.Информация("Строка запуска сборки: " + СтрокаЗапускаСборки);
	Лог.Отладка("тест заменить точку: " + ПреобразоватьПутьСТочкамиКНормальномуПути(".\\..\\ExportScenarios"));
	//@# Конец МА
...
Процедура ОчиститьКаталоги(МассивКаталогов)
	Лог.Отладка("МассивКаталогов = "+МассивКаталогов);
...
Процедура ВыполнитьЗапускВсехСборок(ОсновнойФайлПараметров, ОчищатьКеш = Ложь)
	...
	Для каждого Сборка Из Сборки Цикл
		...
		Лог.Отладка("ОтчетAllure = "+ПараметрыСборки["ОтчетAllure"]);
	...
	КаталогАллюрВсеТесты = СокрЛП(КаталогАллюрВсеТесты);

	КаталогАллюрGenerate = ПреобразоватьПутьСТочкамиКНормальномуПути(".\ServiceBases\AllureReport_HTML");
	КаталогАллюрGenerate = СокрЛП(КаталогАллюрGenerate);
	...
	СтрокаКоманды = "xcopy """ 
		+ КаталогАллюрGenerate + "\history" + """ """ 
		+ КаталогАллюрВсеТесты + "\history"" /H /Y /C /R /S /I";
	retCode = ВыполнитьСкрипт(СтрокаКоманды);
