...
Сценарий: 10.1 Приходная налоговая накладная
	...
	*В реквизитах поставщика по умолчанию устанавливается счет пятьсот двадцать один sil<[100]>один - «Счета к оплате внутри страны»
		И пауза 3
...
//		И Я показываю подсказку 'Счет дебета по умолчанию' у элемента "СубкПоставщика1" в браузере
	...
	*В полях «ТМЦ1» и «ТМЦ2» проставляем соответствующую аналитику счета ТМЦ в нашем случае "Номенклатура"  и "Склад"
...
//	И в таблице "ТМЦ" из выпадающего списка с именем "ТМЦТМЦ2" я выбираю точное значение 'Magazin №1'
	...
	* Единица измерения автоматически проставляется из карточки выбранного ТМЦ
		И пауза 1
		И Я показываю подсказку 'Единица измерения, заполнена автоматически' у ячейки таблицы "ТМЦ" "1" "ТМЦЕдИзм" в браузере

	* В поле «Количество» проставляем количество приходуемых ТМЦ
		И пауза 1
		И Я показываю подсказку 'Единица измерения, заполнена автоматически' у ячейки таблицы "ТМЦ" "1" "ТМЦКоличество" в браузере
	...
	*Если приход осуществлялся в леях, то проставляем общую сумму прихода в леях по данному ТМЦ в ячейку - «Сумма лей». 
		И Пауза 3
		И Я показываю подсказку 'Поле "Сумма (лей)" - Общая сумма прихода' у ячейки таблицы "ТМЦ" "1" "ТМЦСумЛей" в браузере
		И в таблице "ТМЦ" в поле с именем 'ТМЦСумЛей' я ввожу текст '2 500,00'
		И в таблице "ТМЦ" я завершаю редактирование строки

	*Поля «Ставка НДС» sil<[1200]>и «НДС» заполняются автоматически в том случае, когда в карточке ТМЦ был проставлен процент НДС. В противном случае информация вносится вручную;
		И Пауза 1
		...
		|'noFreeze'|'false'|
		И Пауза 2
		И я показываю подсказку "НДС" EnjoyHint у области таблицы "ТМЦ" в браузере
		|'rowNumber'|'1'|
		|'rowNumberEnd'|'1'|
		|'columnTitle'|'НДС'|
		|'columnTitleEnd'|'НДС'|
