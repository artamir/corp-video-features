...
Сценарий: 9.1 Установка цен номенклатуры 
	...
	*Далее - sil<[200]>'Установка цен номенклатуры'
		И Пауза 1
...
//		И Я закрываю окно 'Установка цен номенклатуры * от *'
	...
	*Чтоб указать, что цена включает в себя НДС, устанавливается флаг - "Цены включают НДС"
		И Пауза 1
	...
	*Так же, в поле "Округлять до..." - выбирается до какого разряда округлять
		И Пауза 1
	...
	*Если выбран пункт "Добавить..", то в табличную часть документа добавляются выбранные позиции
		...
		И в таблице "Товары" в поле с именем 'ТоварыЦена' я ввожу текст '6500,00'
	...
	*При проведении документа информация о ценах записывается  в регистр сведений "Ц^ены номенклатуры" по каждой позиции номенклатуры.
		И пауза 1
		И я нажимаю на кнопку с именем 'ФормаОбщаяКомандаПоказатьДвиженияДокумента'
		Тогда открылось окно 'Таблица'
