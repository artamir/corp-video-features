...
Сценарий: 11.1 Импорт тмц в 1С Бухгалтерия для Молодвы 1.2 КОРП
	...
	*Рассмотрим импорт товароматериальных ценностей в программе Один эС Бухгалтерия для Молдовы один точка два версия КОРП 
		И Пауза 1

	*Для начала необходимо убедиться, что на дату таможенной декларации установлен правильный курс валюты.
		И Пауза 1

	*В программе предусмотрена возможность автоматической загрузки курсов валют.
		И Пауза 1

	*Рассмотрим загрузку курсов USD с сайта нац банка. Для этого переходим в раздел "Денежные средства"
		И Пауза 4
	...
	*Документы по импорту тмц можно найти в разделе приходных документов
		И Пауза 1
	...
	*Устанавливим датой документа - дату таможенной декларации
		И Пауза 1
		И в поле с именем 'ДатаДок' я ввожу текст '15062022'
		И я перехожу к следующему реквизиту	
		Тогда открылось окно 'Приход: Приходная налоговая накладная *'

	*Для того, чтоб ввести проформу-инвойс, необходимо установить вид операции - приходная ТэТээН	
	...
	*Курс валюты устанавливается на дату документа
		И Пауза 1
	...
	*Для удобства создадим группу - поставщики импорт
		И Пауза 1	
...
//			Затем клик на картинку "ImportNaimenovanie" со смещением "20" "0"
			//-----------------------
			И я выполняю скрипт SikuliX "ClickImportNaimenovanie"
...
//		И я перехожу к следующему реквизиту
	...
	*
		И я выполняю скрипт SikuliX "UstanovitiMasshtabOkna100.sikuli"
		И Я закрываю окно 'Масштаб'
		

	*Изменим масштаб содержимого окна 
		//Затем Я изменяю масштаб содержимого формы
		Попытка
			И я выполняю скрипт SikuliX "IzmenitiMasshtabOkna"
		Исключение
			И Пауза 1
