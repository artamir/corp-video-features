...
Сценарий: 11.1 Импорт тмц в 1С Бухгалтерия для Молодвы 1.2 КОРП
	...
	*Если это новый контрагент, с которым мы сотрудничаем, то его нужно создать
		...
		И в поле с именем 'Родитель' я ввожу текст 'Поставщики импорт'
		...
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Приход: Приходная ТТН             от  * 0:00:00.  *'
		И в таблице  "ТМЦ" я перехожу на одну строку вверх
		И в таблице  "ТМЦ" я перехожу на одну строку вверх
		...
		И я перехожу к следующему реквизиту
		И я перехожу к следующему реквизиту
		И в таблице "ТМЦ" в поле с именем 'ТМЦСумВал' я ввожу текст '15,00'
		И я перехожу к следующему реквизиту
		И я перехожу к следующему реквизиту
		...
		И Я закрываю окно 'Импорт'
		Тогда открылось окно 'Приход: Приходная ТТН             от  * 0:00:00.  *'
		И в таблице "ТМЦ" я отменяю редактирование строки
